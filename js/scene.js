            ...
            switch (obj.type) {
                ...
                case "mesh":
                    ...
                    for(var k=0; k<faces.length; k++) {
                        ...
                        triangle_num = triangle_num + cur_face.length - 2;
                    }
                    triangle_num = Math.min( triangle_num, MAX_TRIANGLE - count_triangle);
                    if (triangle_num>0) {
                    
                        matStr += "vec3 mesh"+i+"["+triangle_num*3+"];\n";
                        var cur_added = 0;
                        for(var k=0; k<faces.length; k++) {
                            var cur_face = faces[k];
                            for(var v=1; v<cur_face.length-1; v++) {
                                ...
                                funcStr += "mesh"+i+"["+(3*cur_added+0)+"]="+arrayToVec3String(vertices[cur_face[0  ]])+";\n";
                                funcStr += "mesh"+i+"["+(3*cur_added+1)+"]="+arrayToVec3String(vertices[cur_face[v  ]])+";\n";
                                funcStr += "mesh"+i+"["+(3*cur_added+2)+"]="+arrayToVec3String(vertices[cur_face[v+1]])+";\n";
                            
                                cur_added++;
                            }
                        }
                    
                        funcStr += "for (int i=0; i<"+triangle_num+"; i++) {\n";
                        funcStr += "cur_dist = findIntersectionWithTriangle(ray,"+
                            "mesh"+i+"[i*3+0],mesh"+i+"[i*3+1],mesh"+i+"[i*3+2]," +
                            "intersect);\n";
                        funcStr +=
